<?php
require_once('php/security_post.php');
?>
<!DOCTYPE html>
<html lang="es" dir="ltr">
  <head>
    <meta charset="utf-8">
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Scouting | Men&uacute;</title>
    <link rel="stylesheet" type="text/css" href="css/Reset.css">
    <link rel="stylesheet" type="text/css" href="css/general.css">
    <link rel="icon" type="image/png" href="img/nautilus.png">
  </head>
  <body style="background-color: #000;">
    <header>
      <div id="logo-header">
        <h2>Scouting</h2>
        <img src="img/LogoHeader.png" alt="Nautilus 40 10">
      </div>
    </header>
    <section>
      <div class="flexbox">
        <a href="scouting-match.html" class="btn-nav btn">Scouting Match</a>
        <a href="scouting-pit.html" class="btn-nav btn">Scouting Pit</a>
        <a href="" class="btn-nav btn">Resultados</a>
      </div>
    </section>
    
    <footer>
      <div class="centre firma flexbox">
        <a href="https://github.com/DiegoMont" class="firma">&lt;/&gt; with <span id="corazon" class="firma">&#9829;</span> by <span class="firma">DiegoMont</span></a>
      </div>
    </footer>
  </body>
</html>